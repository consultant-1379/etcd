...
*/ -}}
{{- define "eric-data-distributed-coordinator-ed.copyVCTMetadataFields" -}}
...
*/ -}}
{{- define "eric-data-distributed-coordinator-ed.copyVCTAnnotations" -}}
  {{- include "eric-data-distributed-coordinator-ed.copyVCTMetadataFields" (dict "metadataField" "annotations" "vctName" .vctName "customKeys" .customKeys "release" .release "name" .name) | trim -}}
{{- end -}}

{{- define "eric-data-distributed-coordinator-ed.copyVCTLabels" -}}
  {{- $defaultLabels := dict "app.kubernetes.io/instance" .release.Name -}}
  {{- $mergedLabels := include "eric-data-distributed-coordinator-ed.mergeLabels" (dict "location" (printf "%s.copyVCT" .name) "sources" (list $defaultLabels .customKeys)) | fromYaml -}}
  {{- include "eric-data-distributed-coordinator-ed.copyVCTMetadataFields" (dict "metadataField" "labels" "vctName" .vctName "customKeys" $mergedLabels "release" .release "name" .name) | trim -}}
{{- end -}}
